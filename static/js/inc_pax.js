queue()
    ...
    .defer(d3.csv, "static/csv/Record of 5-min delays for MIT (Up to Oct 2018).csv")
    .await(makeGraphs);

function makeGraphs(error, demandJson) {
 ...
 //  "July", "August", "September", "October", "November", "December"
  ...
  "July", "Aug", "Sep", "Oct", "Nov", "Dec"
	...
	incidents.forEach(function(d,i) {
		...
		d.pax = +d["Number of Passenger Affected (>=5 min.)"]
	...
	//////
	// https://github.com/dc-js/dc.js/blob/master/web/examples/sunburst-with-value-accessor.html
	lineGroup = lineDim.group();
	causeGroup = causeDim.group();
	yearGroup = YearDim.group();
	monthGroup = monthDim.group();
	paxReducer = reductio().avg(function(d) { return +d.pax; });
	paxReducer(lineGroup);
	paxReducer(causeGroup);
	paxReducer(yearGroup);
	paxReducer(monthGroup);
    ...
    var paxLineChart = dc.barChart("#pax-by-line");
    var paxCauseChart = dc.barChart("#avg-pax-by-cause");
    var paxMonthChart = dc.barChart("#pax_month_delay_chart");
    var paxYearChart = dc.barChart("#pax_year");
